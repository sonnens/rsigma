...
| `LogSourceExtractor` trait + `FieldLogSourceExtractor` | `LogSourceExtractor` (methods `extract` and `resolve`) derives an event's `LogSource` for conflict-based logsource pruning; `FieldLogSourceExtractor` is the built-in implementation, reading configurable fields plus optional static defaults. Pass an `Arc<dyn LogSourceExtractor>` to `Engine::set_logsource_extractor`. |
...
| `Engine::set_logsource_extractor(Option<Arc<dyn LogSourceExtractor>>)` | Opt into conflict-based logsource pruning: skip rules whose `product`/`service`/`category` (and custom dimensions) conflict with the event's. Off by default, fail-open. Pays off on large mixed-product rule sets. |
